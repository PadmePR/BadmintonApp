...
export default function TeamView({ team, currentUserId, onBack, onTeamUpdated }) {
  ...
  const [teamMatchesId, setTeamMatchesId] = useState(null)
  const [activePlaySessionId, setActivePlaySessionId] = useState(null)
  ...
  function applyMatchData(data) {
    if (!data) { setMatchResult(null); setSavedMeta(null); setTeamMatchesId(null); return }
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (payload.eventType === 'DELETE') {
          setMatchResult(null); setSavedMeta(null); setTeamMatchesId(null)
        } else if (payload.new) {
  ...
  return (
    <div>
      ...
      {/* Render fullscreen PlaySessionPage when active */}
      {activePlaySessionId && (
        <PlaySessionPage sessionId={activePlaySessionId} onClose={() => setActivePlaySessionId(null)} />
      )}
    </div>
  )
}
